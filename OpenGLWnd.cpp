...
COpenGLWnd::~COpenGLWnd() {
  if(!wglDeleteContext(m_hrc)) {
    FormattedErrorAfxMsgBox("wglDeleteContext");
  }
}
...
BOOL
COpenGLWnd::PreCreateWindow(CREATESTRUCT& cs)  {
...
int
COpenGLWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)  {
...
BOOL COpenGLWnd::CreateRGBPalette(HDC hDC)
{
  ...
  if(!pPal) {
    AfxMessageBox(_T("Out of memory for pallette"), MB_OK | MB_ICONSTOP);
    return FALSE;
  }
...
void COpenGLWnd::OnSaveImage()
{
   ...
   //
	...
	_TCHAR szFilter[] = _T("Image Files (*.bmp)|*.bmp|All Files (*.*)|*.*||");

	CFileDialog dlg(FALSE, _T(".bmp"), NULL, 0, szFilter, NULL);
	if(dlg.DoModal() != IDOK)
   {
      delete buffer[0];
      delete buffer;
   ...
   if(!file)
   {
      AfxMessageBox(_T("Unable to write output file file"), IDOK);
      delete buffer[0];
      delete buffer;
...
bool COpenGLWnd::ObtainPixels(GLbyte **&p_pixels)
{
	...
	if (!bResult)
	{
      AfxMessageBox(_T("Unable to select OpenGL rendering context, could not obtain pixel data."));
      p_pixels = NULL;
      return false;
	}
...
int COpenGLWnd::FormattedErrorAfxMsgBox(const char *p_msg)
{
    ...
    CString str;
    str.Format(_T("%s %s"), p_msg, lpMsgBuf);
